...
test('AppleSwitch matches the compact iOS geometry and spring thumb motion', () => {
  ...
  assert.match(track, /transition:\s*background var\(--apple-form-motion-spring\),\s*box-shadow var\(--apple-form-motion-spring\)/)
  assert.match(thumb, /width:\s*14px/)
  assert.match(thumb, /height:\s*14px/)
  assert.match(thumb, /transform:\s*translateX\(0\)/)
  assert.match(thumb, /transition:\s*transform var\(--apple-form-motion-spring\),\s*box-shadow var\(--apple-form-motion-spring\)/)
