...
test('logs summary and list surfaces avoid legacy flat cards', () => {
  ...
  assert.match(logItem, /border-bottom:\s*1px solid var\(--hairline\)/)
...
test('logs console rows expose calm status badges and scan states', () => {
  ...
  assert.match(tableHead, /border-bottom:\s*1px solid var\(--hairline\)/)
  assert.match(tableHead, /background:\s*var\(--card-2\)/)
  assert.match(tableHead, /box-shadow:\s*none/)
  assert.doesNotMatch(tableHead, /material-glass|surface-specular-edge|surface-noise|backdrop-filter/)
  assert.match(logItemHover, /background:\s*var\(--card-hover\)/)
  assert.match(logItemHover, /box-shadow:\s*inset 2px 0 0 var\(--glass-active-border\)/)
  assert.doesNotMatch(logItemHover, /material-glass|surface-specular-edge|surface-noise|backdrop-filter/)
  assert.match(logItemLevel, /content:\s*""/)
  assert.match(logItemLevel, /grid-row:\s*1/)
  assert.match(logItemLevel, /width:\s*2px/)
  assert.match(logItemLevel, /background:\s*var\(--hairline-strong\)/)
