...
test('entities directory controls use shared liquid glass tokens without hardcoded theme fog', () => {
  ...
  assert.doesNotMatch(listCard, /backdrop-filter/)
  assert.doesNotMatch(listCard, /rgba\(255,\s*255,\s*255|rgba\(255,255,255|var\(--surface-card\)/)
  assert.match(listCardHover, /background:\s*var\(--card\)/)
  assert.match(listCardHover, /border-color:\s*var\(--hairline-strong\)/)
  ...
  assert.match(searchClear, /background:\s*var\(--card\)/)
  assert.match(searchClear, /border:\s*1px solid var\(--hairline\)/)
  assert.match(searchClear, /box-shadow:\s*none/)
  assert.doesNotMatch(searchClear, /backdrop-filter/)
  ...
  assert.match(favoriteButton, /background:\s*var\(--card\)/)
  assert.match(favoriteButton, /border:\s*1px solid var\(--glass-edge\)/)
  assert.match(favoriteButton, /color:\s*var\(--badge-error-text\)/)
  assert.match(favoriteButton, /box-shadow:\s*(?:none|var\(--shadow-card\))/)
  assert.doesNotMatch(favoriteButton, /backdrop-filter/)
...
test('entities keyboard focus mirrors hover glass control treatment', () => {
  ...
  assert.match(listCardFocus, /background:\s*var\(--card\)/)
  assert.match(listCardFocus, /border-color:\s*var\(--hairline-strong\)/)
  assert.match(listCardFocus, /box-shadow:\s*var\(--shadow-card\)/)
  ...
  assert.match(darkListCardFocus, /background:\s*var\(--card-hover, var\(--card\)\)/)
  assert.match(darkListCardFocus, /box-shadow:\s*var\(--shadow-card\)/)
})
...
test('entities dark list cards reuse shared material tokens', () => {
  ...
  assert.doesNotMatch(darkListCard, /var\(--surface-specular-edge|var\(--surface-noise\)/)
  assert.doesNotMatch(darkListCard, /var\(--surface-specular-edge|var\(--surface-noise\)/)
  assert.match(darkListCard, /box-shadow:\s*var\(--shadow-card\)/)

  assert.match(darkListCardHover, /border-color:\s*var\(--glass-edge-strong\)/)
  assert.match(darkListCardHover, /background:\s*var\(--card-hover, var\(--card\)\)/)
  assert.match(darkListCardHover, /box-shadow:\s*var\(--shadow-card\)/)
