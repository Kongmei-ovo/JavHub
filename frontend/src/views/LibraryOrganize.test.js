...
test('library organize top chrome uses the shared Apple glass material', () => {
  ...
  assert.match(headerRule, /background:\s*var\(--card\)/)
  assert.match(headerRule, /box-shadow:\s*var\(--shadow-card\)/)
  ...
  assert.match(headerBeforeRule, /background:\s*var\(--card\)/)
  ...
  assert.match(statusRule, /background:\s*var\(--card\)/)
  assert.doesNotMatch(statusRule, /backdrop-filter/)
  ...
  assert.match(setupBannerRule, /background:\s*var\(--card\)/)
  assert.match(setupBannerRule, /box-shadow:\s*var\(--shadow-card\)/)
  assert.doesNotMatch(setupBannerRule, /backdrop-filter/)
...
test('library organize tab counts read as compact console badges', () => {
  ...
  assert.match(tabCountRule, /border:\s*1px solid var\(--hairline\)/)
  assert.match(tabCountRule, /background:\s*var\(--card\)/)
...
test('library organize inventory media placeholders use shared subtle glass material', () => {
  ...
  for (const rule of [posterRule, actorAvatarRule]) {
    assert.match(rule, /background:\s*var\(--card\)/)
...
test('library organize dark theme overlays defer to shared material tokens', () => {
  ...
  assert.match(surfaceRule, /background:\s*var\(--card\)/)
  assert.match(surfaceRule, /box-shadow:\s*var\(--shadow-card\)/)
