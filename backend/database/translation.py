...
def _is_undefined_table_error(exc: Exception) -> bool:
    return isinstance(exc, psycopg2.Error) and getattr(exc, "pgcode", None) == "42P01"
...
def init_translation_db():
    with get_db() as conn:
        ...
        for column in ("maker_json", "label_json"):
            _add_column_if_missing(cursor, f"ALTER TABLE translation_mappings ADD COLUMN IF NOT EXISTS {column} TEXT DEFAULT '{{}}'")
        cursor.execute('''
        ...
        for column in ("started_at", "finished_at"):
            _add_column_if_missing(cursor, f"ALTER TABLE translation_jobs ADD COLUMN IF NOT EXISTS {column} TIMESTAMPTZ")
        cursor.execute('''
...
def _select_translation_row(content_id: str):
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute(
...
def _get_raw(content_id: str) -> Optional[dict]:
    ...
    try:
        row = _select_translation_row(content_id)
...
def _fetch_translation_rows(content_ids: list[str]) -> dict[str, Optional[dict]]:
...
def _load_translation_rows(content_ids: list[str]) -> dict[str, Optional[dict]]:
    if not content_ids:
        return {}
    try:
        return _fetch_translation_rows(content_ids)
    except psycopg2.Error as exc:
        if not _is_undefined_table_error(exc):
            raise
        init_translation_db()
...
def _job_duration_seconds(data: dict) -> int | None:
    ...
    if not finished and data.get("status") in {"running", "pending"}:
        finished = datetime.now(started.tzinfo) if started.tzinfo else datetime.utcnow()
    if not finished:
        return None
    if started.tzinfo and finished.tzinfo is None:
        finished = finished.replace(tzinfo=started.tzinfo)
    elif finished.tzinfo and started.tzinfo is None:
